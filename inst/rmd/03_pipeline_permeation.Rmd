...
### Permeation check features
permeation.features <- 
  c("neg_3879/476.1073mz/18.55min",
    "pos_1081/277.1646mz/8.04min",
    "neg_2133/447.0524mz/8.27min",
    "neg_3428/577.1546mz/15.92min")

permeation.features <- 
  c("neg_592/248.9601mz/0.99min",
    "pos_1963/160.0751mz/13.61min")
...
# Peaks
mzrt.filtered <-
  filter_peaks_summary( peaktable = peaks,
...
if (nrow(peaks.is > 0)){
  mzrt.is.filtered <-
    filter_peaks_summary( peaktable = peaks.is,
                  sample.type = sample.info.pos$Sample.type,
                  rsd.filter = c(),
                  mean.filter = c(),
                  rt.range = rt.range)
  ...
  features.is <- 
    cbind( peaks.is.filtered[, c("mz", "rt", "Identity")], mzrt.is.filtered$summary[, 2:7])
...
Missing was imputed with method KNN. It works in the way that only the feature with 
missing rate <= missing_thres in a group (indicated by group.info) are be imputed 
with KNN group-wise. The rest are imputed imputed with 1/5 of min for each feature.
...
## Calculate group mean after imputation and before norm
data.summary.impute.grp <-
  calculate_summary( data = data.impute$x,
                 type = sample.info.pos$Group,
                 names.suffix = "impute")
...
## Calculate group mean after norm
data.summary.norm.grp <-
  calculate_summary( data = data.norm,
                 type = sample.info.pos$Group,
                 names.suffix = "norm")

features.pos <-
  features %>%
  left_join(data.summary.impute.grp$type.mean, by = "Identity") %>%
  left_join(data.summary.norm.grp$type.mean, by = "Identity")
...
### Merge POS and NEG
#### Check file names from positive and negative mode
```{r}
samples.pos <- sample.info.pos
names(samples.pos) <- paste0(names(sample.info.pos), ".pos")

samples.neg <- sample.info.neg
names(samples.neg) <- paste0(names(sample.info.neg), ".neg")

all_samples <- 
  samples.pos %>% 
  full_join(samples.neg, by = c("Sample.name.pos" = "Sample.name.neg")) %>% 
  rename(Sample.name = Sample.name.pos) %>% 
  mutate(pos_file = !is.na(File.name.pos),
         neg_file = !is.na(File.name.neg))

check_samples <- 
  all_samples %>% filter(!(pos_file & neg_file))
...
#### Check permeation features
```{r}
if (!is.null(permeation.features)){
  check_perm <- 
    data.frame(Identity_mode = permeation.features) %>% 
    left_join(datalist$features, by = "Identity_mode")
...
addWorksheet(wb, "check_samples")
writeData(wb, "check_samples", check_samples)
if (!is.null(permeation.features)){
  addWorksheet(wb, "permeation_features")
  writeData(wb, "permeation_features", check_perm)
}
...
Output table and figures are stored in 'path.result'. If in media, the feature has 
mean value > 500, then it is considered to exist in media. Fold change threshold is 2.
Controlled by:
media.thres = 500
FC_thres = 2
path.result = path.result

Highlight the 4 permeation features on the volcano plot only if all four are 
statistically significant. If that condition is met, the output should be saved 
in a separate folder "permeation".

### Volcano plot with permeation filter and bubbles
```{r}

run_analysis_plot_permeation(data = data,
             media.thres = 500,
             FC_thres = 2,
             permeation.features = permeation.features,
             path.result = path.result,
             map.names = map.names,
